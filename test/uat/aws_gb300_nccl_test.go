...
//
// AWS GB300 uses ComputeDomain + DRA + RoCE (no EFA). Key differences from GB200:
//   - No EFA devices or hugepages
//   - RoCE DRA resource claims (roce.networking.k8s.aws)
//   - Node container removes /opt/amazon EFA plugins to avoid segfaults on RoCE
func TestAWSGB300NCCL(t *testing.T) {
	const (
		certName = "aws-gb300-nccl"
		nodesDir = "testdata/aws/gb300"
		dataDir  = "testdata/aws/gb300/nccl"
	)

	feature := features.New("aws/gb300/nccl-all-reduce").
