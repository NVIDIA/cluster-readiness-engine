...
// on simulated AWS H100 nodes using the exec framework with Megatron-LM.
func TestAWSH100WorkloadRunNemotron5(t *testing.T) {
	const (
		runName  = "aws-h100-workloadrun"
		nodesDir = "testdata/aws/h100"
		dataDir  = "testdata/aws/h100/workloadrun"
	)

	feature := features.New("aws/h100/workloadrun-nemotron5").
		...
		Assess("WorkloadRun succeeds", func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			require.Equal(t, "h100", run.Status.DetectedGPUArchitecture)
