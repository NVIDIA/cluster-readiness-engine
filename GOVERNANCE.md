...
NVCRE is a Kubernetes controller and CLI for GPU cluster burn-in certification: it runs cataloged training and communication workloads against a cluster, measures goodput and bandwidth, and reports failing nodes with a reason. Changes that serve this charter are in scope. Features unrelated to cluster certification are out of scope and are declined, with reasons, in the issue discussion.
