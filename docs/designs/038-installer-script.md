...
the nvcrectl installer (another internal CLI) solves this with a shell installer script served via the GitLab raw file API, enabling a one-liner: `curl -sSL <url> | bash`. We adopt the same pattern for nvcrectl.
...
Create an `installer` bash script at the repository root, adapted from the nvcrectl installer's installer pattern. The script downloads the pre-built binary from the GitHub Releases and installs it.
...
- **the nvcrectl installer pattern**: Proven internal pattern, familiar to NVIDIA operators.
