...
NVCRE is installed today via `nvcrectl setup init`, which applies four phases in order: Kubeflow Trainer v2.1.0, NVCRE CRDs, the controller stack (`config/default`), and LogProfiles (`config/logprofiles`). Platform and GitOps teams need a versioned Helm install path with the same components bundled in one release artifact, published alongside the controller image on NGC.
...
1. **Single Helm chart** at `helm/nvcre/` (Helm-native, not kustomize→Helm conversion):
...
2. **Dual emit from generators** — `make manifests` updates `config/` and syncs into `helm/nvcre/` (CRDs, RBAC, LogProfiles). Kustomize remains for `make deploy` and `nvcrectl` until Helm is default; then deprecate.
...
4. **Publish** to `oci://ghcr.io/nvidia/nvcre` on release tags.
...
- Kubeflow subchart installs into `kubeflow-system` by default; umbrella release namespace is typically `nvcre`.
