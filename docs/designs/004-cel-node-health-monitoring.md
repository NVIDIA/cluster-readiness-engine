...
### 2026-08-05 — detector factory registry removed

The factory registry described under Implementation ("Registry
(`pkg/nodemonitor/registry.go`)") and Rationale ("Pluggable registry") was never
reachable and has been deleted.

`Registry.Register` was called from `cel`'s `init()`, but `Registry.Create` and
`Registry.Has` had no callers: the Job controller constructs the detector
directly via `cel.NewDetector`. The registry could not have been used as
designed, because `Job.spec.nodeHealthMonitor` is a typed struct with a single
`cel` field and carries no detector-name discriminator to look up. Selecting a
detector by name would have required an API change, not a registry lookup.

The extension point itself is unchanged and still stands: adding a non-CEL
detector means implementing `NodeFailureDetector`, adding a typed field to
`NodeHealthMonitor`, and branching on it — the same pattern workload adapters
use (`WorkloadSpec` → `workload.ForSpec`). A compile-time assertion in
`pkg/nodemonitor/cel/detector.go` now enforces interface conformance.
