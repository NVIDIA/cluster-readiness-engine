...
| 8 | **Namespace** | No `templates/namespace.yaml`. nvcrectl sets `CreateNamespace=true`; manual/GitOps installs use `helm --create-namespace` or ArgoCD `CreateNamespace`. |
...
- `make embed-nvcrectl` — legacy YAML only (CRDs, controller, LogProfiles, Trainer); no chart embed.
...
- Pull secrets stay out of Helm release values — nvcrectl creates them client-side (same as YAML path).

## Consequences

### Positive

- `nvcrectl setup init --helm` and `helm install` deploy the same manifest set.
