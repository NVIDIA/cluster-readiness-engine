...
The split is deliberate. Registry attestations are the natural home for something already addressed by digest, and `actions/attest-build-provenance` with `push-to-registry: true` writes them to the registry alongside the subject. A loose sidecar is the right shape for a downloaded file: a user can `curl` the binary and its bundle and verify with cosign alone — no GitHub API, no registry credentials, no network beyond the two downloads and the trust root. That is what makes air-gapped verification work.

**Registry subjects are verified against the registry copy, never against the GitHub Attestations API.** `push-to-registry: true` writes two copies — one to GitHub's attestation store, one to the registry — and they can diverge: a registry-side write can fail, and a referrer can be deleted from GHCR afterwards, neither of which touches the API copy. `gh attestation verify` reads the API copy *by default*, so a gate using its bare form would pass on an image whose registry attestation is absent, while a consumer running `cosign verify-attestation` against that same image gets nothing. Registry verification therefore uses `cosign`, which is registry-native, or `gh attestation verify --bundle-from-oci`. This is the same "verify over the channel the user takes" rule decision 6 applies to downloaded assets; the registry is that channel for images and charts.
