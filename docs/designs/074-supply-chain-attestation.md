...
The release gate closes the other half of that loop: it refuses a legacy-format bundle rather than falling back to reading it. Detached `attest-blob` assets are checked for the protobuf Sigstore `mediaType` (`application/vnd.dev.sigstore.bundle*`) and for a sibling `.sig` before `cosign verify-blob-attestation` runs — the fallback parser is the one that can bypass `--certificate-identity` (GHSA-fx35-mq7g-6g98). Registry subjects are inspected with `cosign tree` for the old `sha256-<digest>.sig` / `.att` sidecar tags, and those tags fail the gate without a subsequent `cosign verify`. The daily job (#273) is still unbuilt; when it lands it should reuse the same checks rather than invent a second discriminator.
