...
awsme solves this with a self-update mechanism that queries GitLab for newer releases and downloads them in-place. We adopt the same pattern for ncrectl, adapted to use the Package Registry (where ncrectl binaries are published) instead of GitLab Releases.
...
1. Fetches the latest version tag from GitLab API.
2. Compares against the running version using semantic versioning.
3. Shows release notes from the GitLab release description.
...
- `fetchLatestVersion()` — GitLab tags API
- `fetchReleaseNotes()` — GitLab releases API
...
- **y/N prompt (not Terraform-style)**: Self-upgrade is lower risk than cluster changes. Simple y/N is sufficient, matching awsme's pattern.
