      ...
      options:
        ...
        resources:
          ...
          requests:
            cpu: "4"
            memory: 32Gi
        # Optional: clone Megatron-LM from an internal mirror instead of
        # github.com (air-gapped or restricted-egress clusters).
...
| `megatronRepo` | string | Optional. Git repository URL for the Megatron-LM source that training categories clone at pod start; defaults to `https://github.com/NVIDIA/Megatron-LM.git`. Point at an internal mirror for air-gapped or restricted-egress clusters (the mirror must serve the pinned `core_v0.15.2` branch). Must include a URL scheme such as `https://` or `ssh://`; scp-style `git@host:path` syntax and URLs containing whitespace or shell metacharacters are rejected (pattern `^[A-Za-z][A-Za-z0-9+.-]*://[A-Za-z0-9._~:/@%+-]+$`). Settable globally on `spec` or per category under `categories[].options`; the per-category value wins |
