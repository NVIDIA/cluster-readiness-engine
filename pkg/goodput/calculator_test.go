...
type interruptionInput struct {
	TrainingTime float64           `yaml:"trainingTime"`
	Event        InterruptionEvent `yaml:"event"`
}

func TestCumulativeMetricsAddInterruption(t *testing.T) {
	p := &testutil.TestCaseParser{
		Subdir:         "cumulative-add-interruption",
		ExpectedSuffix: ".json",
	}
	p.TestDir(t, func(tc *testutil.TestCase) error {
		...
		cm.TrainingTime = input.TrainingTime
		cm.AddInterruption(input.Event)
		b, err := json.MarshalIndent(map[string]any{
			"interruptionCount": cm.InterruptionCount,
			"lostWorkTime":      cm.LostWorkTime,
			"rescheduleTime":    cm.RescheduleTime,
			"resumeTime":        cm.ResumeTime,
			"goodput":           roundTo(cm.Goodput, 6),
		}, "", "  ")
		if err != nil {
			return err
		}
