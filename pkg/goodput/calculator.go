...
// AddInterruption adds a completed interruption event and updates cumulative metrics.
func (c *CumulativeMetrics) AddInterruption(event InterruptionEvent) {
	c.Interruptions = append(c.Interruptions, event)
	c.InterruptionCount = len(c.Interruptions)
	c.LostWorkTime += event.TCh
	c.RescheduleTime += event.TRe
	c.ResumeTime += event.TRm
