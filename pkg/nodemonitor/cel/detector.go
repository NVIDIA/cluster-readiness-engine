...
// NewDetectorFactory returns a factory function for creating CEL detectors.
// The factory expects a string configuration containing the CEL expression.
func NewDetectorFactory() nodemonitor.DetectorFactory {
	return func(config any) (nodemonitor.NodeFailureDetector, error) {
		expr, ok := config.(string)
		if !ok {
			return nil, fmt.Errorf("CEL detector config must be a string expression, got %T", config)
		}
		return NewDetector(expr)
	}
}
...
// init registers the CEL detector factory with the default registry.
func init() {
	nodemonitor.DefaultRegistry.Register(DetectorName, NewDetectorFactory())
}
