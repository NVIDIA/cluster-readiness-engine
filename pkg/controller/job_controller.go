...
func (r *JobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !controllerutil.ContainsFinalizer(job, jobFinalizer) {
		log.Info("Adding finalizer to Job")
...
func (r *JobReconciler) handleDeletion(ctx context.Context, job *burninv1alpha1.Job) (ctrl.Result, error) { //nolint:unparam
	...
	cleanupJobMetrics(job.Namespace, job.Name, job.Labels["cre.nvidia.com/workflow"])
...
func (r *JobReconciler) hasCheckpoint(ctx context.Context, job *burninv1alpha1.Job) bool {
	...
	if err := r.List(ctx, &measurements, client.InNamespace(job.Namespace)); err != nil {
		log.Error(err, "Failed to list GoodputMeasurements")
		return false
	}
	for _, m := range measurements.Items {
		if m.Spec.JobRef.Name == job.Name && m.Status.LastCheckpointStep > 0 {
			return true
		}
	}
	return false
...
func (r *JobReconciler) setJobValidationStatus(ctx context.Context, job *burninv1alpha1.Job, status metav1.ConditionStatus, reason, message string) error {
	...
	if status == metav1.ConditionTrue && len(job.Status.FailedNodes) == 0 {
		job.Status.FailedNodes = noderesults.NodesWithFailureDetails(groupNodeNames(job), ReasonThresholdViolation, message)
	}
	...
	if changed {
		...
		log.Info("ValidationFailed condition set", "reason", reason)
	}
	return nil
}
...
func (r *JobReconciler) setJobHardwareFailed(ctx context.Context, job *burninv1alpha1.Job, reason, message string, failedNodes []burninv1alpha1.FailedNode) error {
	...
	// Check if this is the first hardware failure for this job
	isFirstFailure := len(job.Status.FailedNodes) == 0 && len(failedNodes) > 0

	// Update failed nodes in status
	job.Status.FailedNodes = failedNodes

	// Set HardwareFailed condition independently (not exclusive with other conditions)
	changed := meta.SetStatusCondition(&job.Status.Conditions, metav1.Condition{
...
func (r *JobReconciler) setExclusiveCondition(ctx context.Context, job *burninv1alpha1.Job, conditionType, reason, message string) error {
	...
	// Only job execution states are mutually exclusive
	...
	if changed {
		...
		log.Info("Job status updated", "status", conditionType, "reason", reason)
	}
	return nil
}
...
func (r *JobReconciler) nodeToJobRequests(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	if err := r.List(ctx, podList, client.MatchingFields{podNodeNameIndexField: node.Name}); err != nil {
		// If the index doesn't exist, fall back to filtering manually
	...
	for _, pod := range podList.Items {
		// Skip pods not on this node (for fallback case)
...
// podNodeNameIndexField is the field index for pod node name lookups
