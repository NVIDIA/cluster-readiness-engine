...
type GoodputMeasurementReconciler struct {
	...
	// parsers caches compiled ProfileParsers by LogProfile name.
	parsers map[string]*goodput.ProfileParser
...
func (r *GoodputMeasurementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !controllerutil.ContainsFinalizer(measurement, goodputMeasurementFinalizer) {
		log.Info("Adding finalizer to GoodputMeasurement")
		controllerutil.AddFinalizer(measurement, goodputMeasurementFinalizer)
...
func (r *GoodputMeasurementReconciler) handleRunning(ctx context.Context, measurement *burninv1alpha1.GoodputMeasurement, job *burninv1alpha1.Job) (ctrl.Result, error) { //nolint:gocyclo
	...
	if !state.LastLogFetch.IsZero() {
		sinceTime := metav1.NewTime(state.LastLogFetch.Add(-time.Second))
		opts.SinceTime = &sinceTime
	} else if worker0.Status.StartTime != nil {
		sinceTime := *worker0.Status.StartTime
		opts.SinceTime = &sinceTime
	}
...
func (r *GoodputMeasurementReconciler) handleSucceeded(ctx context.Context, measurement *burninv1alpha1.GoodputMeasurement, job *burninv1alpha1.Job) (ctrl.Result, error) {
	...
	if state != nil && state.TrainingStarted && !cm.TrainingStartTime.IsZero() {
		cm.UpdateTrainingTime(time.Since(cm.TrainingStartTime).Seconds())
	}
...
func (r *GoodputMeasurementReconciler) getOrCreateParser(profile *burninv1alpha1.LogProfile) (*goodput.ProfileParser, error) {
	...
	if r.parsers == nil {
		r.parsers = make(map[string]*goodput.ProfileParser)
	}

	if p, ok := r.parsers[profile.Name]; ok {
		return p, nil
	}
...
// When log-interval > 1, each log line's StepTiming is the per-iteration average,
// so it is multiplied by the step delta to get the total time for that interval.
// Falls back to timestamp gaps when no StepTiming values are available.
