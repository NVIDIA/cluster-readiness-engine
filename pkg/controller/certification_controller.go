...
func (r *CertificationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !controllerutil.ContainsFinalizer(certification, certificationFinalizer) {
		log.Info("Adding finalizer to Certification")
		controllerutil.AddFinalizer(certification, certificationFinalizer)
...
func (r *CertificationReconciler) setExclusiveCondition(ctx context.Context, certification *burninv1alpha1.Certification, conditionType, reason, message string) error {
	...
	for _, ct := range allConditionTypes {
		...
		if meta.SetStatusCondition(&certification.Status.Conditions, metav1.Condition{
			Type:               ct,
			Status:             status,
			Reason:             condReason,
			Message:            condMessage,
			ObservedGeneration: certification.Generation,
		}) {
			changed = true
		}
	}
	...
	if changed {
		if err := r.Status().Update(ctx, certification); err != nil {
			return fmt.Errorf("failed to update Certification status: %w", err)
		}
		log.Info("Certification status updated", "status", conditionType, "reason", reason)
	}
	return nil
}
